...
class PDF extends FPDF {
    function Header() {
        ...
        $this->Cell(80);
        // Title
        $this->Cell(30,10,'Stock Out Table',0,0,'C');
        // Line break
        $this->Ln(20);
    }
...
$query = "SELECT * FROM stockOut, masterStockRecord, stockOutCat WHERE masterStockRecord.id = stockOut.id AND stockOut.stockOutCatId = stockOutCat.id";
...
$pdf->Cell(66,10, 'Description',1,0,'C');
$pdf->Cell(25,10, 'Quantity Used',1,0,'C');
$pdf->Cell(20,10, 'Used By',1,0,'C');
$pdf->Cell(18,10, 'Date',1,1,'C');
...
foreach($result as $row){
    ...
    $pdf->SetX(3);
    $name = $row['name'];
    $StockOutCat = $row['categoryDetail'];
    ...
    $pdf->Cell(30, 10, $StockOutCat,1,0,'C'); 
    $pdf->Cell(66, 10, $Description,1,0,'C'); 
    $pdf->Cell(25, 10, $UsedQ,1,0,'C'); 
    $pdf->Cell(20, 10, $UsedBy,1,0,'C'); 
    $pdf->Cell(18, 10, $Date,1,1,'C'); 
